              ...
              {loading ? (
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-center mb-6">
                    <h3 className="text-xl font-bold text-white mb-2">
                      {activeFilter === 'day' 
                        ...
                        : activeFilter === 'week'
                        ? `This Week - ${new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} to ${new Date(Date.now() + 6 * 24 * 60 * 60 * 1000).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`
                  ...
                  {activeFilter === 'day' ? (
                    ...
                    // Week/Month view with grouped events by day
                    (() => {
                      // Group events by day
                      ...
                      return sortedDays.length > 0 ? sortedDays.map((dayKey) => {
                        const dayEvents = eventsByDay[dayKey];
                        const dayDate = new Date(dayKey);
                        const isToday = dayDate.toDateString() === new Date().toDateString();
                        const isTomorrow = dayDate.toDateString() === new Date(Date.now() + 24 * 60 * 60 * 1000).toDateString();
                        ...
                        return (
                          <div key={dayKey} className="mb-6">
                            <div className="flex items-center gap-3 mb-4">
                              <div className={`w-3 h-3 rounded-full ${isToday ? 'bg-red-500' : isTomorrow ? 'bg-orange-500' : 'bg-purple-500'}`}></div>
                              <h4 className="text-lg font-bold text-white">{dayLabel}</h4>
                              <div className="flex-1 h-px bg-white/10"></div>
                              <span className="text-sm text-gray-400">{dayEvents.length} event{dayEvents.length !== 1 ? 's' : ''}</span>
                            </div>
                                ...
                                >
                                  <div className="flex items-center justify-between">
                                    <div className="flex-1">
                                      <h5 className="text-white font-semibold text-sm mb-1">{event.subject}</h5>
                        ...
                        <div className="text-center py-12">
                          ...
                          <p className="text-xs text-gray-500 mt-1">Your elite schedule is clear</p>
                        </div>
                      );
                    })()
                  )}
